(function() {
                ...
                if (mainData.logos && mainData.logos[0] && mainData.logos[0].file_path) {
                    ...
                    $.get(enRequest, function(enData) {
                        if (enData.logos && enData.logos[0] && enData.logos[0].file_path) {
                            ...
                            insertTextFallback(a.object.activity.render(), e.title || e.name);
                        }
                    });
                } else {
                    insertTextFallback(a.object.activity.render(), e.title || e.name);
    ...
    function insertTextFallback(renderElement, title) {
        var logoBlock = renderElement.find(".full-start-new__logo-block");
        var titleText = $(`<div class="logo-title">${title}</div>`);
        titleText.css({'font-size': '4.5em', 'font-weight': '100', 'text-align': 'center', 'color': '#fff', 'text-shadow': '0 2px 10px rgba(0,0,0,0.6)', 'max-width': '50vw', 'margin-top': '0em', 'margin-bottom': '0em', 'opacity': '0', 'transform': 'translateY(20px)', 'transition': 'all 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)'});
        logoBlock.append(titleText);
        requestAnimationFrame(() => {
            titleText.css({'opacity': '1', 'transform': 'translateY(0) scale(1)'});
        });
    }
})();