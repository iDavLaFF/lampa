/**
 * modification.js — единая точка входа для кастомизаций ветки `idavlampa`.
 * Задачи файла:
 * - Подключает внешние плагины через Lampa.Utils.putScriptAsync
 * - Применяет дефолтные настройки на первом запуске и перезагружает приложение один раз
 * - Упрощает интерфейс: скрывает лишние пункты настроек, элемента шапки и меню
 * - На десктопе скрывает кнопку 18+ и добавляет горячую клавишу (↑↑→→↓↓) для доступа
 */
// Добавляем плагины
const plugins = [
